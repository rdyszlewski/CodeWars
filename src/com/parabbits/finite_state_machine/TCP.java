...
public class TCP {
    ...
    private final static String states = "CLOSED: APP_PASSIVE_OPEN -> LISTEN\n" +
            "CLOSED: APP_ACTIVE_OPEN  -> SYN_SENT\n" +
            "LISTEN: RCV_SYN          -> SYN_RCVD\n" +
            "LISTEN: APP_SEND         -> SYN_SENT\n" +
            "LISTEN: APP_CLOSE        -> CLOSED\n" +
    ...
    public static String traverseStates(String[] events) {
        ...
        Map<String, Map<String, String>> statesMachine = parseStates(states);
        for(String event: events){
            if(statesMachine.containsKey(currentState) && statesMachine.get(currentState).containsKey(event)){
                currentState = statesMachine.get(currentState).get(event);
            } else {
                return "ERROR";
            }
        }
    ...
    private static Map<String, Map<String, String>> parseStates(String states){
        Map<String, Map<String, String>> statesMachine = new ConcurrentHashMap<>();
        Pattern pattern = Pattern.compile("(\\w*): (\\w*).*-> (\\w*)");
        Matcher matcher = pattern.matcher(states);
        while(matcher.find()){
            String initState = matcher.group(1);
            String command = matcher.group(2);
            String resultState = matcher.group(3);
            if(!statesMachine.containsKey(initState)){
                statesMachine.put(initState, new ConcurrentHashMap<>());
            }
            statesMachine.get(initState).put(command, resultState);
        }
